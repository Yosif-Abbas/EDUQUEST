...
import { useEnrollInCourse } from '../../hooks/courses/useEnrollIncourse';
...
function WishlistItem({ item, wishlist, isLoadingWishlist }) {
  ...
  const { enrolledCourses, isLoading: isLoadingEnrolledCourses } = useEnrolledCourses(
    currentUser?.id,
  );
  ...
  const isEnrolled =
    !isLoadingEnrolledCourses && enrolledCourses?.some((c) => c.course_id.id === couseId);
  ...
  useEffect(() => {
    ...
    if (wishlist?.length > 0) {
      const isCourseInWishlist = wishlist.some((item) => item.course_id.id === couseId);
  ...
  return (
    <li className={`student-wishlist-item px-2 ${isEnrolled ? 'bg-green-100' : ''}`}>
      ...
      <div className="col-span-4 flex flex-col justify-center gap-1 md:col-span-2 lg:col-span-3 xl:flex-row xl:items-center xl:justify-end xl:gap-2">
        {isEnrolled ? (
          ...
          >
            {isEnrollingInCourse ? <Spinner size={30} color="white" /> : 'Enroll Now'}
          </button>
        ) : (
          <>
