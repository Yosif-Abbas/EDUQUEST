...
function Instructor({ instructor }) {
  ...
  // students count
  const teacherStudents = courses.reduce((acc, course) => course.students_enrolled + acc, 0);
  ...
  // average rating
  const teacherRating = (
    courses.reduce((acc, course) => course.rating + acc, 0) / teacherCoursesCount
  ).toFixed(1);
  ...
  return (
    <div className="flex items-start gap-5 p-4">
      ...
      <div className="w-full text-xs font-normal text-gray-500">
        <h2 className="my-2 text-lg font-semibold text-black">{instructorFullName}</h2>
