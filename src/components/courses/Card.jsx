...
import CourseSubject from './courseDetails/CourseSubject';
...
function Card({ course, className }) {
  ...
  return (
    <li className={`h-fit w-full border border-gray-300 bg-white ${className} max-w-90`}>
      <figure
      ...
      <div className="flex items-center justify-between border-t border-[#8C94A355] p-4 text-xs font-medium lg:text-lg">
        ...
        <span className="list-icon text-[#4E5566]">
          ...
          {course.students_enrolled} <span className="text-[#4E5566aa]">Students</span>
        </span>
      </div>
    </li>
  );
